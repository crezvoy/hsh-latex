...
case "$HSH_ACTION" in
    bundle-out|clone)
	yes | "$HSH_ROOT/bin/pk" install texlive-latex-recommended
	yes | "$HSH_ROOT/bin/pk" install texlive-bibtex-extra
	yes | "$HSH_ROOT/bin/pk" install texlive-lang-french
	yes | "$HSH_ROOT/bin/pk" install texlive-latex-extra
	yes | "$HSH_ROOT/bin/pk" install texlive-xetex
	;;
esac
